# -*- coding: utf-8 -*-
"""
bart_dialog_generator_with_gt_strategy.py
============================
...
CUDA_VISIBLE_DEVICES=2 python bart_dialog_generator_with_gt_strategy.py --eval_only --output_dir outputs/dialog_eval_gt_strategy

# 그래디언트 누적을 사용한 대용량 배치 학습 (실효 배치 크기 32)
CUDA_VISIBLE_DEVICES=3 python bart_dialog_generator_with_gt_strategy.py --gradient_accumulation_steps 2 --output_dir outputs/dialog_batch_32_gt_strategy
"""
...
class DialogGenDataset(torch.utils.data.Dataset):
    ...
    대화 맥락(context)과 전략(strategy)을 입력으로, 시스템 응답을 출력으로 하는 데이터셋
    """
    ...
    def __init__(
        ...
        for ex in raw:
            ...
            for turn_idx, turn in enumerate(dialog):
                ...
                curr = dialog[turn_idx]
                strategy = curr["strategy"]
                ctx_parts.append(f"{SPECIAL_TOKENS["sys"]}{SPECIAL_TOKENS[strategy]}")
